...
def save_to_excel(data, supplier_data, output_file):
    ...
    try:
        ...
        flat_supplier_data = [
            {
                ...
                "Юридический адрес": supplier["legalAddress"],  # Добавляем юридический адрес
                "Торговая марка": supplier["trademark"],
                "КПП": supplier["kpp"],  # Добавляем КПП
                "Номер регистрации": supplier["taxpayerCode"],  # Добавляем номер регистрации
        ...
        # Настройка ширины столбцов для листа Products
        ...
        # Настройка ширины столбцов для листа Suppliers
        supplier_worksheet = workbook['Suppliers']
        ...
        for col_idx, col_name in enumerate(supplier_df.columns, start=1):
            ...
            if col_name in supplier_columns_to_widen_one:
                ...
                logger.debug(f"Установлена ширина {first_width} для столбца {col_name} ({column_letter}) в листе Suppliers")
            elif col_name in supplier_columns_to_widen_two:
                supplier_worksheet.column_dimensions[column_letter].width = second_width
                logger.debug(f"Установлена ширина {second_width} для столбца {col_name} ({column_letter}) в листе Suppliers")
            elif col_name in supplier_columns_to_widen_three:
                supplier_worksheet.column_dimensions[column_letter].width = third_width
                logger.debug(f"Установлена ширина {third_width} для столбца {col_name} ({column_letter}) в листе Suppliers")
